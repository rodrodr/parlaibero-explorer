/* ===== src/worker/sha256.js ===== */
/* 2REP_Standalone · worker/sha256.js
 *
 * - Sha256: SHA-256 incremental en JavaScript puro (FIPS 180-4). No depende de crypto.subtle, que no es
 *   incremental: así el archivo se resume por trozos sin tenerlo entero en memoria.
 * - huellaTrozo: huella rápida NO criptográfica de un trozo, para detectar que el archivo cambió en disco
 *   entre dos lecturas (Firefox no avisa: lee los bytes nuevos).
 */
(function (R2) {
  'use strict';
  ...
  class Sha256 {
    constructor() {
      ...
      this.pendiente = new Uint8Array(64); // bytes que aún no completan un bloque
      this.nPendiente = 0;
      this.longitud = 0; // bytes procesados en total
      this.terminado = false;
    }

    /** Añade bytes (Uint8Array). Devuelve this. */
    update(datos) {
    ...
    /** Resumen final en hexadecimal (64 caracteres). Solo puede pedirse una vez. */
    hex() {
  ...
  /** SHA-256 de un Uint8Array completo, en hexadecimal. */
  function hex(bytes) {
    return new Sha256().update(bytes).hex();
  }

  /**
   * Huella de 64 bits (16 caracteres hex) de un trozo. Dos carriles de 32 bits sobre las palabras pares e
   * impares, con h = (h ^ palabra) · impar: cada paso es una biyección del estado, así que cambiar una sola
   * palabra cambia SIEMPRE la huella; varios cambios colisionan con probabilidad del orden de 2^-32.
   * Solo se compara en la misma máquina (usa el orden de bytes nativo).
   */
  function huellaTrozo(u8) {
    ...
    } else { // vista desalineada: se copia a un búfer propio
