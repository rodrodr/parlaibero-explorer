// Tercera ronda: la muestra se sacó de menciones6_2.json (v6.2, semilla 31, 8 por categoría, revision7/); se compara con
// menciones6.json (versión en curso) casando por posición.
const fs = require('fs');
...
for (const p of 'AR BR CL CO CR DO EC ES GT MX PA PE PT PY SV UY'.split(' ')) {
  if (!fs.existsSync(`revision7/${p}.json`)) { console.log(`${p}: (sin revisión)`); continue; }
  ...
  sample.forEach((m, k) => {
    ...
    if (estado !== 'igual') det.push(`${p} #${k + 1} ${r.veredicto.padEnd(10)} ${estado.padEnd(10)} ${m.texto} : ${quien(m)} → ${quien(n6)}${r.no_deberia_contar ? ' [no debía contar]' : ''}`);
  });
...
fs.writeFileSync('rescore3_detalle.txt', det.join('\n'));
