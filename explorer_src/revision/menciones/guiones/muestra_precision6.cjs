// Muestra aleatoria (semilla fija) de menciones por categoría de la salida v6 (menciones6.json), con su contexto y cómo se
// resolvieron, para revisarlas a mano. Uso: node muestra_precision6.cjs <carpeta> [n por categoría] [semilla]
const fs = require('fs'), path = require('path');
const [DIR, N = '8', SEMILLA = '23'] = process.argv.slice(2);
const { menciones } = JSON.parse(fs.readFileSync(path.join(DIR, 'menciones6.json'), 'utf8'));
